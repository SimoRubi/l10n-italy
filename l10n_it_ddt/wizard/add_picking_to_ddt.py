    ...
    def add_to_ddt(self):
        ...
        for picking in pickings:
            ...
            if picking.ddt_ids and \
                ...
                raise UserError(
                    _(f"Picking {picking.name} already in ddt"))
            elif (
                current_ddt_shipping_partner != self.ddt_id.partner_shipping_id
            ):
                raise UserError(
                    _(f"Selected Picking {picking.name} have"
                      f" different Partner"))
            if picking.sale_id:
